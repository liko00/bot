...
async def process_visits(message: types.Message, state: FSMContext):
    ...
    await message.reply("Enter days of the week(Monday - 1, Friday -5 ):")
    await Form.next()

@dp.message_handler(state=Form.days)
async def process_days(message: types.Message, state: FSMContext):
    await state.update_data(days=message.text)
    await message.reply("Enter time(24-hour format):")
    await Form.next()
...
async def process_time(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        conn.commit()
        await message.reply(f"User data for {username} has been successfully saved.")
